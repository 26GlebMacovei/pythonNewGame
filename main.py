...
class Game (pg.sprite.Sprite):
    ...
    def draw(self):
        ...
        self.draw_text(self.screen, str(self.score), 22, WIDTH / 2, 15, BLACK)
    ...
    def show_start_screen(self):
        ...
        for i in range(WIDTH//2):
                self.screen.fill(BLUE)
                self.draw_text(self.screen, 'Up to ', 64, -100+i, HEIGHT / 4, WHITE)
                self.draw_text(self.screen, ' the space', 64, 570 - i, HEIGHT / 4, WHITE)
                pg.display.flip()
        pg.display.flip()
        waiting = True
        while waiting:
